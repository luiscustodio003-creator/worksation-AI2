...
Trata a gestão de conhecimento: ingestão de ficheiros, extracção, metadados, indexação, recuperação e construção de contexto. Na unidade 9.1 foi criado o contrato declarativo do subsistema (`wsai2.knowledge`): `KnowledgeKind` (document/extract/note), `KnowledgeMetadata` (fonte, idioma, autor, etiquetas) e `KnowledgeRecord` (imutável, validado à criação), além da versão de contrato `KNOWLEDGE_CONTRACT_VERSION = "1.0"`. Na unidade 9.2 foi criado o `KnowledgeRegistry` (catálogo de admissão por id único, padrão dos registos das Fases 4–8), dependente apenas de `wsai2.core`. Na unidade 9.3 foi criada a extracção heurística de metadados (`KnowledgeMetadataExtractor`: detecção determinista de idioma e derivação de etiquetas, preservando a proveniência declarada), sem I/O nem modelos. Na unidade 9.4 foi criado o `KnowledgeIndex` (índice invertido em memória: consulta por termos com pesos de campo e ordenação determinista, reindexação por substituição), também sem I/O, persistência ou embeddings. Na unidade 9.5 foi criado o `KnowledgeStore` (persistência SQLite local por id com metadados serializados em JSON — decisão de armazenamento documentada; os registos persistidos reconstroem registo e índice no arranque); a recuperação semântica via embeddings fica para fase posterior com decisão própria. Na unidade 9.6 foi criada a construção de contexto (`ContextBuilder`/`KnowledgeContext`): pacote textual determinista por relevância do índice — títulos, excertos com elipse, proveniência e etiquetas — fechando o circuito contrato→registo→metadados→índice→persistência→contexto do núcleo reversível da Fase 9. Na unidade 9.7 foi criada a ingestão de ficheiros (`FileIngestor`): leitura lexical de ficheiros de texto do projecto (.md/.py/.txt/.json/.toml/.yaml/.yml) em registos do contrato com proveniência relativa e metadados derivados — sem embeddings. O enriquecimento semântico (embeddings via Model/Provider) é o ponto de decisão seguinte e exige planeamento próprio, sem antecipar motores de embeddings neste contrato.
